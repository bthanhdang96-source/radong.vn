...
test('buildPageCopy uses phổ quát wording when price is stable', () => {
  ...
  assert.match(copy.bodyHtml, /So với các khu vực khác/i)
  assert.match(copy.bodyText, /độc giả|bạn đọc/i)
  assert.doesNotMatch(copy.bodyText, /bà con|thương lái|nông dân|hệ thống|tự động|điểm dữ liệu|điểm quan sát/i)
})
